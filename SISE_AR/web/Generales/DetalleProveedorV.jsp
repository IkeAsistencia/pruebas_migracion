...
<html>
    <head>
        <title>Detalle Proveedor Viajes</title>
    </head>
    <body class="cssBody">
        ...
        <%
            MyUtil.InicializaParametrosC(244, Integer.parseInt(StrclUsr));
        %><%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion", "fnLimpiaExtra()")%>
        <INPUT id='URLBACK' name='URLBACK' type='hidden' value='<%=request.getRequestURL().substring(0, request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleProveedorV.jsp?"%>'>
        <%
        ...
        <INPUT id='clAreaOperativa' name='clAreaOperativa' type='hidden' value='3'>
        ...
        <%=MyUtil.ObjComboC("Rubro", "clGiro", rs.getString("dsGiro"), true, true, 310, 120, "", "Select clGiro, dsGiro from cGiro where clAreaOperativa=3 order by dsGiro", "", "", 30, true, true)%>
        ...
        <%=MyUtil.ObjComboC("Tipo de Proveedor", "clTipoProveedor", rs.getString("dsTipoProveedor"), true, true, 25, 200, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 3 order by dsTipoProveedor", "", "", 20, true, true)%>
        <%=MyUtil.ObjInput("Prioridad", "Prioridad", rs.getString("Prioridad"), true, true, 200, 200, "", true, true, 5)%>
        <%=MyUtil.DoBlock("Detalle de Proveedor de Viajes", 160, 0)%>
        ...
        <%=MyUtil.ObjComboC("Banco", "clBanco", rs.getString("dsBanco"), true, true, 25, yPos, "", "Select * from cBanco where Activo=1 order by Nombre", "", "", 50, false, false)%>
        ...
        <%=MyUtil.ObjComboC("Rubro", "clGiro", "", true, true, 310, 120, "", "Select clGiro, dsGiro from cGiro where clAreaOperativa=3 order by dsGiro", "", "", 30, true, true)%>
        ...
        <%=MyUtil.ObjComboC("Tipo de Proveedor", "clTipoProveedor", "", true, true, 25, 200, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 3 order by dsTipoProveedor", "", "", 20, true, true)%>
        ...
        <%=MyUtil.ObjInput("", "CuentaBan", "", true, true, 325, yPos, "", false, false, 20)%>
