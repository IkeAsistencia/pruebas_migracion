...
<html>
    ...
    <body class="cssBody">
        ...
        StrSql1.append(" Select NombreRZ from cProveedor where clProveedor=").append(StrclProveedor);
        ResultSet rsRZ = UtileriasBDF.rsSQLNP( StrSql1.toString());
        StrSql1.delete(0,StrSql1.length());
        
        if(rsRZ.next())
        {
            StrNombreRZ = rsRZ.getString("NombreRZ");
        }
        ...
        String StrclPaginaWeb = "490";
        session.setAttribute("clPaginaWebP",StrclPaginaWeb);
        %>
        <SCRIPT>fnOpenLinks()</script>
        <%
        MyUtil.InicializaParametrosC(490,Integer.parseInt(StrclUsrApp));    // se checan permisos de alta,baja,cambio,consulta de esta pagina
        %>
        <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion","")%>
        <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleCentroAtencionR.jsp?'>"%>
        <%
        if (rs.next())
        {
        %>
        ...
        <%=MyUtil.ObjInput("Nombre Razon Social","NombreRZVTR",StrNombreRZ,false,false,20,100,StrNombreRZ,false,false,50)%>
        <%=MyUtil.ObjInput("Nombre Operativo","NombreOpe",StrNomOpe,false,false,310,100,StrNomOpe,false,false,70)%>
        
        <%=MyUtil.ObjInput("Nombre Centro Atención","NmbCentroAtecion",rs.getString("NmbCentroAtecion"),true,true,20,140,"",true,true,40)%>
        <%=MyUtil.ObjComboC("Tipo Centro de Atención","clTipoCentroAtencion",rs.getString("dsTipoCentroAtencion"),true,true,310,140,"2","SELECT * FROM cTipoCentroAtencion","","",50,true,true)%>
        ...
        <%=MyUtil.DoBlock("Detalle Centro de Atención Redes Médicas",80,40)%>
        ...
        <%=MyUtil.ObjComboC("Tipo Centro de Atención","clTipoCentroAtencion","",true,true,310,140,"2","SELECT * FROM cTipoCentroAtencion ","","",50,true,true)%>
        ...
        <%session.setAttribute("clCentroAtencion","");%>
