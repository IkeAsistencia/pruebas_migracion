...
<html>
    <head>
        <title>Detalle Proveedor Redes Médicas</title>
    ...
    <body class="cssBody">
        ...
        <%
            MyUtil.InicializaParametrosC(480, Integer.parseInt(StrclUsr));
        %>
        <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion", "fnLimpiaExtra()")%>
        <INPUT id='URLBACK' name='URLBACK' type='hidden' value='<%=request.getRequestURL().substring(0, request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleProveedorR.jsp?"%>'>
        <%
            if (rs.next()) {
        ...
        <INPUT id='clAreaOperativa' name='clAreaOperativa' type='hidden' value='9'>

        <%=MyUtil.ObjInput("Nombre", "NombreRZ", rs.getString("NombreRZ"), true, true, 25, 80, "", true, true, 50)%>
        <%=MyUtil.ObjInput("Nombre En Operación", "NombreOpe", rs.getString("NombreOpe"), true, true, 310, 80, "", true, true, 50)%>
        ...
        <%=MyUtil.ObjComboC("Tipo de Proveedor", "clTipoProveedor", rs.getString("dsTipoProveedor"), true, true, 310, 160, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 5 order by dsTipoProveedor", "", "", 20, true, true)%>
        <%=MyUtil.ObjInput("Prioridad", "Prioridad", rs.getString("Prioridad"), false, false, 150, 160, "1", false, false, 5)%>
        <%=MyUtil.DoBlock("Detalle de Proveedor Redes Médicas")%>
        <%yPos = 240;%>
        ...
        <%=MyUtil.ObjComboMem("Provincia", "CodEnt", dsEntFed, CodEnt, cbEntidad.GeneraHTML(20, dsEntFed), true, true, 25, yPos, "", "fnLlenaMunicipios()", "", 20, true, true)%>
        <%=MyUtil.ObjComboMem("Localidad", "CodMD", dsMunDel, StrCodMD, cbEntidad.GeneraHTMLMD(30, CodEnt, dsMunDel), true, true, 410, yPos, "", "", "", 20, true, true)%>
        <%yPos += 40;%>
        ...
        <%=MyUtil.ObjInput("Email 1", "Email1", rs.getString("Email1"), true, true, 25, 430, "", false, false, 40)%>
        <%=MyUtil.ObjInput("Email 2", "Email2", rs.getString("Email2"), true, true, 250, 430, "", false, false, 40)%>
        <%=MyUtil.ObjInput("Email 3", "Email3", rs.getString("Email3"), true, true, 475, 430, "", false, false, 40)%>
        ...
        <%=MyUtil.ObjInput("Nombre En Operación", "NombreOpe", "", true, true, 310, 80, "", true, true, 50)%>
        <%=MyUtil.ObjInput("Titular", "Titular", "", true, true, 25, 120, "", true, true, 50)%>
        <%=MyUtil.ObjComboC("Rubro", "clGiro", "", true, true, 310, 120, "", "Select clGiro, dsGiro from cGiro where Activo=1 where clAreaOperativa=5 order by dsGiro", "", "", 20, true, true)%>
        ...
        <%=MyUtil.ObjComboC("Tipo de Proveedor", "clTipoProveedor", "", true, true, 310, 160, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 5 order by dsTipoProveedor", "", "", 20, true, true)%>
        ...
        <%=MyUtil.DoBlock("Ubicación del Proveedor", 170, 0)%>
        <%yPos += 80;%>

        <%=MyUtil.ObjComboC("Banco", "clBanco", "", true, true, 25, yPos, "", "Select * from cBanco order by Nombre", "", "", 50, false, false)%>
        ...
        <%=MyUtil.ObjInput("Email 1", "Email1", "", true, true, 25, 430, "", false, false, 40)%>
        <%=MyUtil.ObjInput("Email 2", "Email2", "", true, true, 250, 430, "", false, false, 40)%>
        <%=MyUtil.ObjInput("Email 3", "Email3", "", true, true, 475, 430, "", false, false, 40)%>
        <%=MyUtil.DoBlock("Banco del Proveedor", 0, 0)%>
        <%yPos += 110;%>
