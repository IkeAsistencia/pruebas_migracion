...
<html>
    ...
    <body class="cssBody">
        ...
        String StrclPaginaWeb = "491";
        session.setAttribute("clPaginaWebP",StrclPaginaWeb);
        %><SCRIPT>fnOpenLinks()</script><%       
                
        MyUtil.InicializaParametrosC(491,Integer.parseInt(StrclUsrApp));    
        %>
        <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion","")%>
        <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleSubservxProvF.jsp?'>"%>
        ...
        <%=MyUtil.DoBlock("Detalle de Subservicio por Proveedor Funerario",0,30)%>
