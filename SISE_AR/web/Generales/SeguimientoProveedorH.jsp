...
<html>
<head>        
    <title>DetalleSeguimientoProveedorH</title>
</head>
...
<body class="cssBody" onload="">
    ...
    String StrclPaginaWeb = "797";
    session.setAttribute("clPaginaWebP",StrclPaginaWeb);
    %>
    <script>fnOpenLinks()</script>
    
    <%MyUtil.InicializaParametrosC(797,Integer.parseInt(StrclUsr));
    %>
    <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion","")%>
    
    <INPUT id='URLBACK' name='URLBACK' type='hidden' value='<%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="SeguimientoProveedorH.jsp?"%>'>
    <%
    int iY = 40;
    %>          
