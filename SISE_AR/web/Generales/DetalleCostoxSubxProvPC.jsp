...
<html>
    ...
    <body class="cssBody" >
        ...
        <INPUT id='URLBACK' name='URLBACK' type='hidden' value='<%=request.getRequestURL().substring(0, request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleCostoxSubxProvPC.jsp?"%>'>
        ...
        <%=MyUtil.ObjComboC("Subservicio", "clSubServicio", CP != null ? CP.getDsSubservicio() : "", true, true, 250, 120, "0", "SELECT clSubServicio, dsSubServicio FROM cSubServicio where clServicio ='" + StrclServicio + "' ORDER BY dsSubServicio ", "fnLlenaConceptos(" + StrclAreaOperativa + ")", "", 50, true, true)%>
        <%=MyUtil.ObjComboC("Concepto", "clConcepto", CP != null ? CP.getDsConcepto() : "", true, true, 30, 160, "0", "SELECT clConcepto, dsConcepto FROM cConceptoCosto where clAreaOperativa = '" + StrclAreaOperativa + "' ORDER BY dsConcepto", "", "", 50, true, true)%>
        <%=MyUtil.ObjInput("<b>Costo Base</b>", "Costo", CP != null ? CP.getCosto() : "", true, true, 30, 200, "0", true, true, 10)%>
