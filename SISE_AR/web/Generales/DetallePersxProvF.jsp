...
<%  
    ...
    StrSql.append(" where PxP.clPersonalxProv =").append(StrclPersonalxProv);
     ...
     StrSql.delete(0,StrSql.length());
       ...
       String StrclPaginaWeb = "485";
       session.setAttribute("clPaginaWebP",StrclPaginaWeb);
        %>
        <SCRIPT>fnOpenLinks()</script>
        <%
       MyUtil.InicializaParametrosC(485,Integer.parseInt(StrclUsrApp));    // se checan permisos de alta,baja,cambio,consulta de esta pagina 
        %>
       <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion","")%>
     <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="DetallePersxProvF.jsp?'>"%>
       <%
       ...
       if (rs.next()) {
        ...
        StrNomOpe=null;
        %>
          <%=MyUtil.DoBlock("Detalle de Personal por Proveedor (FUNERARIO)",190,0)%>
