...
<%@ page import = "Seguridad.SeguridadC,Combos.cbEstatus,com.ike.concierge.DAOConciergeG,com.ike.concierge.to.ConciergeG,com.ike.concierge.DAOCSExperienciasPriceless,com.ike.concierge.to.CSExperienciasPriceless,com.ike.concierge.DAOReferenciasxAsist,com.ike.concierge.to.ReferenciasxAsist,java.sql.ResultSet,Utilerias.UtileriasBDF"%>
<html>
    <head>
        <title>Experiencias Priceless</title>
    ...
    <body class="cssBody" onload="fnOnLoad();">
        ...
        <%
            ...
            DAOCSExperienciasPriceless DAOE = null;
            CSExperienciasPriceless EP = null;
            ...
            if (strclUsr != null) {
                ...
                DAOE = new DAOCSExperienciasPriceless();
                EP = DAOE.getCSExperienciasPriceless(StrclAsistencia);
            ...
            //<<<<<<<<<<<< Servlet Generico >>>>>>>>>>>
            String Store = "";
            Store = "st_GuardaCSExpPrice,st_ActualizaCSExpPrice"; //!!!!!!!!!!!!!!!!!!!!!!!
            ...
            strclCiudad = EP != null ? EP.getClCiudadExp() : "0";
            strdsExperiencia = EP != null ? EP.getDsExperiencia() : "";
            strExpCancelada = EP != null ? EP.getExpCancelada() : "0";
            strFechaExpCancelada = EP != null ? EP.getFechaExpCancelada() : "";
            strClCSExpPriceless = EP != null ? EP.getClCSExpPriceless() : "";
        ...
        <INPUT id='clCSExpPriceless' name='clCSExpPriceless' type='hidden' value='<%=strClCSExpPriceless%>'>
        ...
        <input id="SecuenciaG" name="SecuenciaG" type="hidden" value="clConcierge,FechaApAsist,DescripcionEvento,clTipoBen,clCiudad,clExperiencia,FechaE,NLugaresDisponibles,NAdultos,Ninos,Edades,comentExp,wList,clTipoPago,NomBanco,NombreTC,NumeroTC,Expira,SecC,Confirmo,NConfirmo,PCancel,NuInf,DomFiscal,clUsrApp">
        <input id="SecuenciaA" name="SecuenciaA" type="hidden" value="clCSExpPriceless,clAsistencia,DescripcionEvento,clExperiencia,NAdultos,Ninos,Edades,comentExp,wList,expCancelada,clTipoPago,NomBanco,NombreTC,Confirmo,NConfirmo,PCancel,NuInf,DomFiscal,NAdultosTMP,clUsrApp">
        ...
        <%=MyUtil.ObjComboC("Beneficio Priceless", "clTipoBen", EP != null ? EP.getDsTipoBen() : "", true, false, 30, 280, "", "select clTipoBen, dsTipoBen from csctipobenpriceless where Activo=1 order by dsTipoBen", "fnLlenaCiudadExpAjax(this.value);", "", 20, true, false)%>
        <%=MyUtil.ObjComboCDiv("Ciudad", "clCiudad", EP != null ? EP.getDsCiudadExp() : "", true, false, 230, 280, "", "select clCiudad,dsCiudad from cscciudadexppriceless where clTipoBen = " + strclTipoBen + " order by dsCiudad", "fnLlenaExperienciaAjax(this.value)", "", 20, true, false, "CiudadExpDiv")%>
        <%=MyUtil.ObjComboCDiv("Experiencia / Acceso", "clExperiencia", EP != null ? EP.getDsExperiencia() : "", true, false, 430, 280, "", "select clExperiencia, dsExperiencia from cscexperiencias where clCiudad = " + strclCiudad + " and Activo=1 order by dsExperiencia", "fnLlenaLocalidades()", "", 20, true, false, "ExperienciadDiv")%>
        ...
        <div class='VTable' id="divBtnCancelaExp" style="position:absolute; z-index:501; left:460px; top:410px; ">
            <input class='cBtn' type='button' value='Cancelar Experiencia/Acceso' onClick="fnCancelaExp('<%=strdsExperiencia%>');">
        </div>
        ...
        <%=MyUtil.DoBlock("Datos Generales de la Experiencia", -110, 70)%>
        ...
        <script type="text/javascript" >
            ...
            function fnOnLoad() {
                ...
                if (document.all.clCSExpPriceless.value != "") {
            ...
            function fnLlenaLocalidades() {
                ...
                if ((document.all.clCiudad.value != 0) && (document.all.clExperiencia.value.toString() != "")) {
                    var pstrCadena = "CSValidacionesExpPriceless.jsp?clCiudad=" + document.all.clCiudad.value +
                            "&clExperiencia=" + document.all.clExperiencia.value;
            ...
            function fnAccionesAlta() {
                if (document.all.Action.value == 1) {
                    ...
                    document.all.clExperiencia.value = "0";
                    document.all.Edades.readOnly = true;
                    document.all.divWList.style.visibility = 'hidden'
                    document.all.divLblCancela.style.visibility = 'hidden';
                }
            ...
            function fnLlenaExperienciaAjax(value) {
                IDCombo = 'clExperiencia';
                Label = 'Experiencia / Acceso';
                IdDiv = 'ExperienciadDiv';
                FnCombo = 'fnLlenaLocalidades()';
                URL = "../../servlet/Utilerias.LlenaCombosAjax?";
                var strConsulta = "st_GetExpCS '" + document.all.clCiudad.value + "'";
                var Cadena = "strSQL=" + strConsulta + "&strName=clExperienciaC";
                Cadena += "&Opcion=" + value + "&IdCombo=" + IDCombo + "&Label=" + Label + "&FnCombo=" + FnCombo;
                fnLLenaInput(URL, Cadena, IdDiv);
            ...
            function fnValidaLocalidades() {
                if ((isNaN(document.all.NAdultos.value) == true) || (document.all.NAdultos.value.toString().match(/\./))) {
                    ...
                    if (document.all.clTipoBen.value.toString() == "" || document.all.clTipoBen.value.toString() == "0") {
                        ...
                        document.all.NAdultos.value = "";
                    } else if (document.all.clExperiencia.value.toString() == "" || document.all.clExperiencia.value.toString() == "0") {
            ...
            function fnValidaFecha() {
                if (document.all.clExperiencia.value.toString() == "" || document.all.clExperiencia.value.toString() == "0") {
                    alert("Debe seleccionar Experiencia / Acceso.");
            ...
            function fnCancelaExp(dsExperiencia) {
                ...
                if (resp == true) {
                    window.open('CSCancelaExp.jsp?clCSExpPriceless=' + document.all.clCSExpPriceless.value + '&valProceso=1&NAdultos=' + document.all.NAdultos.value, 'Hist', 'scrollbars=yes,status=yes,width=650,height=300');
                }
            }
                ...
                if (document.all.wList.value == "0" && document.all.expCancelada.value == "0") {
                    ...
                    if (respWL == true) {
                        if (parseInt(document.all.NLugaresDisponibles.value) >= parseInt(document.all.NAdultos.value)) {     //alert("VOY A DESCONTAR !!");                                        
                            window.open('CSCancelaExp.jsp?clCSExpPriceless=' + document.all.clCSExpPriceless.value + '&valProceso=2&NAdultos=' + document.all.NAdultos.value, 'Hist', 'scrollbars=yes,status=yes,width=650,height=300');
            ...
            function fnEliminaWL() {
                ...
                if (WL == true) {
                    window.open('CSCancelaExp.jsp?clCSExpPriceless=' + document.all.clCSExpPriceless.value + '&valProceso=3&NAdultos=' + document.all.NAdultos.value, 'Hist', 'scrollbars=yes,status=yes,width=650,height=300');
                } else {
