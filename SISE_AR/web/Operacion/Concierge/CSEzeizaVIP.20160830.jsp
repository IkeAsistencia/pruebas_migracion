...
<html>
    ...
    <body class="cssBody" onload="fnAccionesAlta();">
        ...
        <div class='VTable' style='position:absolute; z-index:25; left:400px; top:15px;'><p align="center"><font color="navy" face="Arial" size="3" ><b><i>Ezeiza VIP</i></b></font><br></p></div>
        <div class='VTable' style='position:absolute; z-index:25; left:460px; top:93px;'><p align="center"><font color="navy" face="Arial" size="2" ><b><i>Automática</i></b></font><br></p></div>
                <%
        ...
        <%=MyUtil.ObjTextArea("Descripción del evento", "DescripcionEvento", EVIP != null ? EVIP.getDescripcion().trim() : "", "77", "7", true, true, 30, 120, "", true, true)%>
        <div style="position:  relative; z-index: 80; left: 460px; top: 100px;">
            ...
            <div style="position: absolute; left: 40px; top: 8px; text-align: justify">
                ...
                <b class="terminos">aceptar términos<br>y condiciones.</b>
            </div>
        </div>
        ...
        <div id="btnEnvio" name="btnEnvio" style='position:absolute; z-index:25; left:745px; top:200px;'>
            ...
            <INPUT type='button' value='ENVIAR TÉRMINOS Y CONDICIONES' onClick='fnEnviaCondicionado();' class='cBtn'>
        </div>
        <%=MyUtil.DoBlock("Términos y Condiciones", -50, 100)%>

        <%=MyUtil.ObjInput("Nombre del Banco", "NombreBanco", StrNombreBanco, true, true, 30, 265, StrNombreBanco, true, true, 45)%>
        <%=MyUtil.ObjInput("Número de BIN", "NumeroBin", StrNumeroBin, true, true, 285, 265, StrNumeroBin, true, true, 45)%>
        ...
        <%=MyUtil.ObjInput("Teléfono", "TelNU", StrTelNU, true, true, 210, 345, StrTelNU, true, true, 40)%>
        <!--%=MyUtil.ObjInput("Correo Electrónico", "CorreoNU", StrEmailComercialNU, true, true, 440, 345, StrEmailComercialNU, true, true, 40, "fnvalidaCorreo();")%-->
        ...
        <%=MyUtil.ObjComboC("Aerolínea", "clAerolinea_I", EVIP != null ? EVIP.getDsAerolinea_I() : "", true, true, 30, 420, "", "select clAerolinea, dsAerolinea from CScAerolinea order by (2)", "", "", 5, false, false)%>
        ...
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX1_I", EVIP != null ? EVIP.getBinPAX1_I() : "", true, true, 895, 455, "", false, false, 15, "VerificaNumerico(document.all.BinPAX1_I);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX2_I", EVIP != null ? EVIP.getBinPAX2_I() : "", true, true, 895, 495, "", false, false, 15, "VerificaNumerico(document.all.BinPAX2_I);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX3_I", EVIP != null ? EVIP.getBinPAX3_I() : "", true, true, 895, 535, "", false, false, 15, "VerificaNumerico(document.all.BinPAX3_I);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX4_I", EVIP != null ? EVIP.getBinPAX4_I() : "", true, true, 895, 575, "", false, false, 15, "VerificaNumerico(document.all.BinPAX4_I);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX5_I", EVIP != null ? EVIP.getBinPAX5_I() : "", true, true, 895, 615, "", false, false, 15, "VerificaNumerico(document.all.BinPAX5_I);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX6_I", EVIP != null ? EVIP.getBinPAX6_I() : "", true, true, 895, 655, "", false, false, 15, "VerificaNumerico(document.all.BinPAX6_I);")%>
        <%=MyUtil.DoBlock("Información del Vuelo  <b>PARTIDA</b>", -85, -10)%>
        <!--    INFORMACION DE VUELO DE PARTIDA FIN    -->

        <!--    INFORMACION DE VUELO DE LLEGADA     -->
        <%=MyUtil.ObjComboC("Aerolínea", "clAerolinea_R", EVIP != null ? EVIP.getDsAerolinea_R() : "", true, true, 30, 730, "", "select clAerolinea, dsAerolinea from CScAerolinea order by (2)", "", "", 5, false, false)%>
        ...
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX1_R", EVIP != null ? EVIP.getBinPAX1_R() : "", true, true, 895, 765, "", false, false, 15, "VerificaNumerico(document.all.BinPAX1_R);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX2_R", EVIP != null ? EVIP.getBinPAX2_R() : "", true, true, 895, 805, "", false, false, 15, "VerificaNumerico(document.all.BinPAX2_R);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX3_R", EVIP != null ? EVIP.getBinPAX3_R() : "", true, true, 895, 845, "", false, false, 15, "VerificaNumerico(document.all.BinPAX3_R);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX4_R", EVIP != null ? EVIP.getBinPAX4_R() : "", true, true, 895, 885, "", false, false, 15, "VerificaNumerico(document.all.BinPAX4_R);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX5_R", EVIP != null ? EVIP.getBinPAX5_R() : "", true, true, 895, 925, "", false, false, 15, "VerificaNumerico(document.all.BinPAX5_R);")%>
        <%=MyUtil.ObjInput("Número de BIN", "BinPAX6_R", EVIP != null ? EVIP.getBinPAX6_R() : "", true, true, 895, 965, "", false, false, 15, "VerificaNumerico(document.all.BinPAX6_R);")%>
        <%=MyUtil.DoBlock("Información del Vuelo <b>LLEGADA</b>", -85, -10)%>
        ...
        <div onmouseover="return overlib('<b>INTRODUCE UNA BREVE DESCRIPCIÓN DEL AUTO.</b>',CENTER);" onmouseout="return nd();">
        ...
        <%=MyUtil.ObjInput("Número de TC:", "NumeroTC", EVIP != null ? EVIP.getNumeroTC() : "", false, false, 400, 1080, "", false, false, 50, "if(this.readOnly==false){fnValMask(this,document.all.NumeroTCMsk.value,this.name)};fnValidaPrefijoTC(this.value);")%>
        ...
        <%=MyUtil.ObjInput("Pol. Cancelación", "PCancel", EVIP != null ? EVIP.getPCancel() : "", true, true, 710, 1120, "", false, false, 30)%>
        <%=MyUtil.ObjChkBox("N/U inf.:", "NuInf", EVIP != null ? EVIP.getNuInf() : "", true, true, 890, 1118, "", "SI", "NO", "")%>
        <%=MyUtil.ObjTextArea("Domicilio del Tarjetahabiente (máx. 500 caracteres)", "DomFiscal", EVIP != null ? EVIP.getDomFiscal() : "", "80", "8", true, true, 400, 1160, "", false, false)%>
        <div id="btnAutorizar" name="btnAutorizar" style="position: absolute; z-index: 115; left: 850px; top: 1190px;">
        ...
        <%=MyUtil.DoBlock("Datos de Facturación", -80, 80)%>
        ...
        <script type="text/javascript" >
            ...
            function fnVerificaMailCondicionado(Activo,Correo){
                if(document.all[Activo].value==1){
                    if(document.all[Correo].value==''){
                        ...
                        alert('Dirección no válida para envio de Términos y Condiciones. Debe ingresar un email desde el Perfil del Usuario.');                      
                    }
                }
            }

            function fnHabilitaBIN_I(){
                if (document.all.THABPAX1_I.value==1){document.all.BinPAX1_I.disabled=false;} else {document.all.BinPAX1_I.disabled=true; document.all.BinPAX1_I.value='';}
                if (document.all.THABPAX2_I.value==1){document.all.BinPAX2_I.disabled=false;} else {document.all.BinPAX2_I.disabled=true; document.all.BinPAX2_I.value='';}
                if (document.all.THABPAX3_I.value==1){document.all.BinPAX3_I.disabled=false;} else {document.all.BinPAX3_I.disabled=true; document.all.BinPAX3_I.value='';}
                if (document.all.THABPAX4_I.value==1){document.all.BinPAX4_I.disabled=false;} else {document.all.BinPAX4_I.disabled=true; document.all.BinPAX4_I.value='';}
                if (document.all.THABPAX5_I.value==1){document.all.BinPAX5_I.disabled=false;} else {document.all.BinPAX5_I.disabled=true; document.all.BinPAX5_I.value='';}
                if (document.all.THABPAX6_I.value==1){document.all.BinPAX6_I.disabled=false;} else {document.all.BinPAX6_I.disabled=true; document.all.BinPAX6_I.value='';}
            }

            function fnHabilitaBIN_R(){
                if (document.all.THABPAX1_R.value==1){document.all.BinPAX1_R.disabled=false;} else {document.all.BinPAX1_R.disabled=true; document.all.BinPAX1_R.value='';}
                if (document.all.THABPAX2_R.value==1){document.all.BinPAX2_R.disabled=false;} else {document.all.BinPAX2_R.disabled=true; document.all.BinPAX2_R.value='';}
                if (document.all.THABPAX3_R.value==1){document.all.BinPAX3_R.disabled=false;} else {document.all.BinPAX3_R.disabled=true; document.all.BinPAX3_R.value='';}
                if (document.all.THABPAX4_R.value==1){document.all.BinPAX4_R.disabled=false;} else {document.all.BinPAX4_R.disabled=true; document.all.BinPAX4_R.value='';}
                if (document.all.THABPAX5_R.value==1){document.all.BinPAX5_R.disabled=false;} else {document.all.BinPAX5_R.disabled=true; document.all.BinPAX5_R.value='';}
                if (document.all.THABPAX6_R.value==1){document.all.BinPAX6_R.disabled=false;} else {document.all.BinPAX6_R.disabled=true; document.all.BinPAX6_R.value='';}
            }
                ...
                if(document.all.Action.value==1){
                    if(document.all.AvisoPrivacidad.value==1){
                        if(document.all.EnviaTCC.value=='1' || document.all.EnviaTCP.value=='1' || document.all.EnviaTCA.value=='1' || document.all.EnviaTCO.value=='1' || document.all.EnviaTCX1.value=='1'){
                            alert("Se enviarán Términos y Condiciones a las direcciones seleccionadas.");
                        } 
                        ...
                        msgVal = msgVal + " El usuario debe aceptar los Términos y Condiciones para poder guardar la Asistencia.";
            ...
            function VerificaNumerico(Campo){
                if (isNaN(Campo.value)==true){
                    alert('El dato debe ser numérico, favor de verificar.');
                    Campo.focus();
                }
            }
            ...
            function fnEnviaCondicionado(){
                if (document.all.AsistenciaVTR.value==""){
                    alert("Para poder enviar los Términos y Condiciones, primero debes guardar la Asistencia.")
                } else {
                    if(document.all.EnviaTCC.value=='1' || document.all.EnviaTCP.value=='1' || document.all.EnviaTCA.value=='1' || document.all.EnviaTCO.value=='1' || document.all.EnviaTCX1.value=='1'){
                        var Resp = confirm ("¿Deseas enviar los Términos y Condiciones?")
                        if (Resp){
                        ...
                        alert('Debes seleccionar un email válido.');
                    }
                }
            }
            ...
            function fnValidaEnvio(Mensaje){
                ...
                if (Mensaje==1){
                    alert("Se ha programado exitosamente el envío de Términos y Condiciones.");
                }
                if (Mensaje==2){
                    alert("Se encontró un problema al registrar el envio. Favor de contactar a su Administrador.");
                }
            }
