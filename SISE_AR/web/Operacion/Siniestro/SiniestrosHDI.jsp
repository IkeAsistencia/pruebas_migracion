...
<html>
    ...
    <body class="cssBody">
        ...
        <%
            ...
            StrSql1.append(" coalesce(RD.dsRespuesta,'') as dsRespuesta,");
            ...
            StrSql1.append(" From AsistenciaSiniestroHDI AL");
            StrSql1.append(" Left Join cRespuesta RD ON (AL.clEnvioMail=RD.clRespuesta)");
        ...
        <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0, request.getRequestURL().lastIndexOf("/") + 1)%><%="SiniestrosHDI.jsp?'>"%>
        <% if (rs.next()) { %>
            <script>document.all.btnAlta.disabled = true;</script>
            <script>document.all.btnElimina.disabled = true;</script>
            <INPUT id='clExpediente' name='clExpediente' type='hidden' value='<%=StrclExpediente%>'>
            <%=MyUtil.ObjComboC("Envío Mail", "clEnvioMail", rs.getString("dsRespuesta"), true, true, 30, 80, "", "Select clRespuesta,dsRespuesta from cRespuesta where clRespuesta in (1,2)", "", "", 140, true, true)%>
        ...
        <% } else { %>
            ...
            <script>document.all.btnElimina.disabled = true;</script>
            <INPUT id='clExpediente' name='clExpediente' type='hidden' value='<%=StrclExpediente%>'>
            <%=MyUtil.ObjComboC("Envío Mail", "clEnvioMail", "", true, true, 30, 80, "", "Select clRespuesta,dsRespuesta from cRespuesta where clRespuesta in (1,2)", "", "", 140, true, true)%>
