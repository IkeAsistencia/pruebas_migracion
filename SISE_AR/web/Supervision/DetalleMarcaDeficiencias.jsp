...
<html>
    ...
    <body class="cssBody">
        ...
        <%
            ...
            StrSql.append(" Select clareaOperativa From Expediente E inner join cServicio S on (E.clservicio= S.clServicio) where clExpediente= ").append(StrclExpediente);
            ...
            StrSql.append("select clEstatusDef from DeficienciasxExpediente where clDeficienciaxExpediente=").append(StrclDeficienciaxExpediente);
        ...
        <% if (rs.next()) { %>
            ...
            <%=MyUtil.ObjComboC("Proveedor con Deficiencia", "clProveedorDef", rs.getString("Proveedor"), true, true, 280, 110, "", "st_ComboProvxExp " + StrclExpediente, "fnLlenaDeficiencias(2)", "", 50, false, false)%>
            <%=MyUtil.ObjComboC("Deficiencia", "clDeficiencia", rs.getString("dsDeficiencia"), true, true, 30, 150, "", "select * from cDeficiencias where clDeficiencia=" + rs.getString("clDeficiencia"), "", "", 50, true, true)%>
            <%=MyUtil.ObjTextArea("Observaciones del Supervisor", "ObservacionesSup", rs.getString("ObservacionesSup"), "100", "5", true, true, 30, 190, "", false, false)%>
            <%=MyUtil.ObjComboC("Estatus Deficiencia", "clEstatusDef", rs.getString("dsEstatusDef"), false, true, 30, 280, "1", "SELECT clEstatusDef,dsEstatusDef FROM CESTATUSDEF", "", "", 50, false, true)%>
        <% } else { %>
            ...
            <%=MyUtil.ObjComboC("Proveedor con Deficiencia", "clProveedorDef", "", true, true, 280, 110, "", "st_ComboProvxExp " + StrclExpediente, "fnLlenaDeficiencias(2)", "", 50, false, false)%>
            <%=MyUtil.ObjComboC("Deficiencia", "clDeficiencia", "", true, true, 30, 150, "", "select * from cDeficiencias where clDeficiencia=-1", "", "", 50, true, true)%>
            <%=MyUtil.ObjTextArea("Observaciones del Supervisor", "ObservacionesSup", "", "100", "5", true, true, 30, 190, "", false, false)%>
            <%=MyUtil.ObjComboC("Estatus Deficiencia", "clEstatusDef", "", false, true, 30, 280, "1", "SELECT clEstatusDef,dsEstatusDef FROM CESTATUSDEF", "", "", 50, false, true)%>
        <%}%>      
        ...
        <script>
            ...
            function fnLlenaAreaDef() {
                var strConsulta = "sp_LlenaComboAreaDef '" + document.all.clTipoDeficienciaVTR.value + "'," + <%= StrclAreaOperativa%>;
            ...
            function fnLlenaDeficiencias(pCampoSolicitado) {
                ...
                if (((pCampoSolicitado == 1) && (Tipo == 3)) || (pCampoSolicitado == 2)) {
                    var strConsulta = "sp_LlenaComboDeficiencias '" + document.all.clExpediente.value + "','" + document.all.clAreaDef.value
                            + "','" + document.all.clUsrAppDef.value + "','" + document.all.clProveedorDef.value + "'," + <%= StrclAreaOperativa%>;
                    var pstrCadena = "../servlet/Utilerias.LlenaCombos?strSQL=" + strConsulta;
