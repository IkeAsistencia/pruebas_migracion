...
public class cbCuenta {
    ...
    private synchronized static boolean loadComboList()
    {
         ...
         {        
             ...
             try{
                 ResultSet rs = UtileriasBDF.rsSQLNP("SELECT clCuenta clLlave ,Nombre dsDesc FROM cCuenta where dsCuenta is not null and activo = 1 order by dsCuenta");
                 while(rs.next())
                 {
             ...
             {
                 System.out.println("Entre a Load de Cuenta");
             }
         }
         return true;
    }
    ...
    public static String GeneraHTML(int pSize, String pValue){
        ...
        HashMap hshMap=cbCuenta.getComboHash();
