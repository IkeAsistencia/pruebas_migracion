...
public class cbEstatus {
    ...
    private static boolean loadComboList() {
        ...
        try {
            ResultSet rs = UtileriasBDF.rsSQLNP("SELECT clEstatus clLlave ,dsEstatus dsDesc FROM cEstatus ORDER BY dsEstatus");
            while (rs.next()) {
    ...
    public static String GeneraHTML(int pSize, String pValue) {
        ...
        HashMap hshMap = cbEstatus.getComboHash();
