...
<%  
   ...
   if (rs.next()) {
        ...
        <%=MyUtil.ObjInput("Fecha de Expedición <br>(AAAA/MM/DD)","FechaExped",rs.getString("FechaExped"),true,true,200,200,"",true,true,25,"if(this.readOnly==false){fnValidaFecha(1," + rs.getString("AnioFecExp") + ")}")%>
        <%=MyUtil.ObjInput("Fecha de Exhibición <br>(AAAA/MM/DD)","FechaExhib",rs.getString("FechaExhib"),false,false,370,200,StrFecha,true,true,25,"if(this.readOnly==false){fnValidaFecha(2," + rs.getString("AnioFecExh") + ")}")%>
        <%=MyUtil.ObjComboC("Tipo Liberación", "clTipoLibera",rs.getString("dsTipoLibera"),true,true,520,210,"","Select clTipoLibera, dsTipoLibera From cTipoLibera","","",50,true,true)%><%
	...
	<INPUT id='clProveedorExhibe' name='clProveedorExhibe' type='hidden' value='<%=strclProveedorExhibe%>'><%
        ...
        <%=MyUtil.ObjComboC("Estatus Caución", "clEstatusCaucion",rs.getString("dsEstatus"),false,false,30,290,StrclEstatusCau,"Select clEstatus, dsEstatus From cEstatusCaucion order by dsEstatus","","",50,true,true)%>                                
        <%=MyUtil.ObjTextArea("Observaciones","Observaciones",rs.getString("Observaciones"),"59","3",false,false,30,330,"",false,false)%>
        <%=MyUtil.DoBlock("Datos de la Caución",0,20)%><%
}
   else { 
        ...
        <%=MyUtil.ObjInput("Monto Reparación <br>Daño","MontoRepDan","",true,true,30,70,"0",false,false,15,"fnActualizaMontos(document.all.MontoRepDan)")%>
        <%=MyUtil.ObjInput("Monto Sanción <br>Pecunaria","MontoSanPec","",true,true,170,70,"0",false,false,15,"fnActualizaMontos(document.all.MontoSanPec)")%>
        <%=MyUtil.ObjInput("Monto Obligación <br>Procesal o Libertad","MontoObProc","",true,true,320,70,"0",false,false,15,"fnActualizaMontos(document.all.MontoObProc)")%>
        <%=MyUtil.ObjInput("Monto Suspensión <br>Acto Reclamado","MontoSusAct","",true,true,470,70,"0",false,false,15,"fnActualizaMontos(document.all.MontoSusAct)")%>
        ...
        <%=MyUtil.ObjInput("Folio de Caución","FolioCaucionVTR","",false,false,30,160,"",false,false,10)%>
        <%=MyUtil.ObjInput("Monto Total Caución","MontoCaucionVTR","",false,false,200,160,"",false,false,15)%>
        <%=MyUtil.ObjComboC("Tipo de pago", "clTipoPagoCaucion","",false,false,370,160,"","st_TipoPagoCaucion","","",50,false,false)%>
        <%=MyUtil.ObjInput("Fecha de Expedición <br>(AAAA/MM/DD)","FechaExped","",true,true,30,200,"",true,true,25,"if(this.readOnly==false){fnValidaFecha(1,0)}")%>
        <%=MyUtil.ObjInput("Fecha de Exhibición <br>(AAAA/MM/DD)","FechaExhib","",false,false,200,200,StrFecha,true,true,25,"if(this.readOnly==false){fnValidaFecha(2,0)}")%>
        <%=MyUtil.ObjComboC("Tipo Liberación", "clTipoLibera","",true,true,370,210,"","Select clTipoLibera, dsTipoLibera From cTipoLibera","","",50,true,true)%>
	...
	<INPUT id='clProveedorExhibe' name='clProveedorExhibe' type='hidden' value=''><%
        ...
        <%=MyUtil.ObjComboC("Estatus Caución", "clEstatusCaucion",rs4.getString("dsEstatus"),false,false,30,290,rs4.getString("clEstatus"),"Select clEstatus, dsEstatus from cEstatusCaucion","","",50,true,true)%>
        <%=MyUtil.ObjTextArea("Observaciones","Observaciones","","59","3",false,false,30,330,"",false,false)%>
        <%=MyUtil.DoBlock("Datos de la Caución",60,20)%><%
    }%>
