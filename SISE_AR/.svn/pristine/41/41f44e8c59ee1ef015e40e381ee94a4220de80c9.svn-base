...
<html>
    ...
    <body class="cssBody">
        ...
        StrclPaginaWeb = "258";
        MyUtil.InicializaParametrosC(258,Integer.parseInt(StrclUsrApp));    // se checan permisos de alta,baja,cambio,consulta de esta pagina

        session.setAttribute("clPaginaWebP",StrclPaginaWeb);
        %>
        <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion","")%>
        <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleCostoxSubservxEFKM.jsp?'>"%>
        <%
        if (rs.next())
        {
        ...
        <%=MyUtil.ObjComboC("Concepto","clConcepto",rs.getString("dsConcepto"),true,false,30,100,"","Select clConcepto, dsConcepto From cConceptoCosto Where clAreaOperativa= 1 Order by dsConcepto","","",100,true,false)%>
        <%=MyUtil.ObjComboC("Servicio","clServicio",rs.getString("dsServicio"),true,false,30,140,"","Select clServicio,dsServicio From cServicio Where clAreaOperativa =1 Order by dsServicio","fnLlenaSubServicios()","",60,true,false)%>
        ...
        <%=MyUtil.ObjComboC("Concepto","clConcepto","",true,false,30,100,"","Select clConcepto, dsConcepto From cConceptoCosto Where clAreaOperativa= 1 Order by dsConcepto","","",100,true,false)%>
        <%=MyUtil.ObjComboC("Servicio","clServicio","",true,false,30,140,"","Select clServicio,dsServicio From cServicio Where clAreaOperativa =1 Order by dsServicio","fnLlenaSubServicios()","",60,true,false)%>
