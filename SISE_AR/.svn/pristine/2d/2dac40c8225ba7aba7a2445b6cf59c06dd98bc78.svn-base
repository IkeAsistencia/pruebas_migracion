...
<html>
    ...
    <body class="cssBody">
        ...
        StrclPaginaWeb = "5060";
        MyUtil.InicializaParametrosC(5060,Integer.parseInt(StrclUsrApp));    // se checan permisos de alta,baja,cambio,consulta de esta pagina
        
        session.setAttribute("clPaginaWebP",StrclPaginaWeb);
        
        %><%=MyUtil.doMenuAct("../../servlet/Utilerias.EjecutaAccionAsist","")%>
        <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="OdontologiaDomi.jsp?'>"%> 
        ...
        <%=MyUtil.DoBlock("Consultoría Odontológica a Domicilio",-30,0)%>                
