...
public class cbTipoClave {
    ...
    private synchronized static boolean loadComboList()
    {
      ...
      {        
         ...
         try{
             ...
             ResultSet rs = UtileriasBDF.rsSQLNP("SELECT clTipoClave clLlave ,dsTipoClave dsDesc FROM cTipoClave ORDER BY dsTipoClave");
             while(rs.next())
             {   
         ...
         {
             System.out.println("Entre a Load de Tipo Clave");
         }
      }
      return true;
    }
    ...
    public static String GeneraHTML(int pSize, String pValue){
        ...
        HashMap hshMap=cbTipoClave.getComboHash();
