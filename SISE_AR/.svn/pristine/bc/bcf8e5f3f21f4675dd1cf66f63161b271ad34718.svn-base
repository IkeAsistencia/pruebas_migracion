 ...
 * cbReclamacion.java
 *
 * Created on 20 de octubre de 2005, 01:23 PM
 *
...
public class cbReclamacion {
    ...
    private synchronized static boolean loadComboList()
    {
      if(isConfigured == false)
      {
         ...
         try{
             ...
             ResultSet rs = UtileriasBDF.rsSQLNP("SELECT clTipoReclamacion clLlave, dsTipoReclamacion dsDesc FROM cTipoReclamacion order by dsTipoReclamacion");
             while(rs.next())
             {
         ...
         {
             System.out.println("Entre a Load de Estatus de Reclamación");
         }
      }
         return true;
    }
    ...
    public static String GeneraHTML(int pSize, String pValue){
        ...
        HashMap hshMap=cbReclamacion.getComboHash();
