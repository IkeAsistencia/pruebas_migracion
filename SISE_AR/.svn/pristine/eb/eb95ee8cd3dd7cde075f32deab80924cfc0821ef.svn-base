...
<html>
    ...
    <body class="cssBody">
        ...
        String StrclPaginaWeb = "493";
        session.setAttribute("clPaginaWebP",StrclPaginaWeb);
        %><SCRIPT>fnOpenLinks()</script><%
        
        
        MyUtil.InicializaParametrosC(493,Integer.parseInt(StrclUsrApp)); 
        %><%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion","")%>
        <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleSubservxProvR.jsp?'>"%>
        ...
        <%=MyUtil.DoBlock("Detalle de Subservicio por Proveedor Redes Médicas",0,30)%>
