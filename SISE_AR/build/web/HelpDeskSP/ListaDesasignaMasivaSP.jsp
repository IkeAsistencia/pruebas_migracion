...
<html>
    ...
    <body class="cssBody" topmargin=150>
        ...
        if (session.getAttribute("clUsrAppSP")!= null) {
              StrclUsrAppAs = session.getAttribute("clUsrAppSP").toString();
        }
        ...
        if (request.getParameter("clPerifericoF")!= null) {
            StrclPeriferico = request.getParameter("clPerifericoF").toString();
        }
        if (StrclPeriferico.compareToIgnoreCase("")==0){
            StrclPeriferico = "0";
        }
        ...
        MyUtil.InicializaParametrosC(1222,Integer.parseInt(StrclUsrApp));%>   
        
        <div style='position:absolute; z-index:303; left:30px; top:10px'>
            <b><font color="#423A9E"><b>EQUIPOS DE RESPONSIVA MASIVA </b></b>
        </div> 
        
        <%
           ResultSet rsUsuario = UtileriasBDF.rsSQLNP("Select Nombre From cUsrAppSP where clUsrAppSP = "+ StrclUsrAppAs);
           String Strresponsable="";
           if(rsUsuario.next()){
               Strresponsable = rsUsuario.getString("Nombre");
           }
               
        %>
        
        <form method='get' action='ListaDesasignaMasivaSP.jsp'>
            <%=MyUtil.ObjInput("Responsable de Equipo","Usuario",Strresponsable,false,false,30,70,"Selec",false,false,45)%>
           <%--  <%=MyUtil.ObjComboC("Area Operativa ","clAreaOperativa","",true,true,30,70,"","Select * from cAreaoperativaSP","","",20,true,true)%>
           <%=MyUtil.ObjInput("No. Asistencia","clPerifericoF","",true,true,370,70,"",false,false,15)%>
            ...
            <div style="visibility:hidden" id="check">
                <%=MyUtil.ObjChkBox("Seleccionar Todos","chkSeleccionar","0",false,true,350,70,"0","SI","NO","fnSelecc(this.checked)")%>    
            </div>           
               <%=MyUtil.DoBlock("Equipos Asignados",60,0)%>
            <script>
               // document.all.clAreaOperativaC.disabled= false;
               // document.all.clAreaOperativaC.readOnly= false;
                document.all.Usuario.readOnly= true;
                document.all.Usuario.disabled= true;
        ...
        StrSql.append("sp_ListPerifericosDesAsignamasiva ").append("''").append(",").append("''").append(",").append("''").append(",").append("''").append(",").append("''").append(", '").append(StrclUsrAppAs).append("'");
       // StrSql.append(",'" ).append(StrFechaAsignacionIni).append("','").append(StrFechaAsignacionFin).append("'");
        ...
        %>
        <div class='VTable' style='position:absolute; z-index:25; left:230px; top:230px;'>
            <p style="font-family:arial;color:#000066;font-size:20px;">La búsqueda no regresó Equipos Asignados</p>
        </div>
        <%
        return;
        }
        ...
        <div id="divi" style='position:absolute; z-index:25; left:20px; top:120px;'>
            <form target='WinSave' method='post' action='DesasignacionMasivaSP.jsp'>
                <table><tr class='cssTitDet'><td>Seleccionado</td><td>Area Operativa</td><td>Asignado a</td><td>Equipo</td><td>Marca</td><td>No. de Serie</td><td>No.</td></tr>
                <%
                while(rs.next()) {
                %>
