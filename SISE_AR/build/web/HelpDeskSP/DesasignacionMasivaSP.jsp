...
<html>
    ...
    <body class="cssBody">
     ...
     if (session.getAttribute("clUsrAppSP")!= null) {
        ...
        StrSql.append("sp_DesAsignacionMasivaEquipo '").append(StrclUsrAppSP).append("', '").append(StPerifericosElegidos).append("'");
        UtileriasBDF.ejecutaSQLNP(StrSql.toString());
        System.out.println("a desasignar:  "+  StrSql.toString() );
        StrSql=null;
        %>
        <script> window.opener.fnValidaResponse(1,'ListaDesasignaMasivaSP.jsp?');window.close();</script>
        <%}catch(Exception e){
        e.printStackTrace();
        }
