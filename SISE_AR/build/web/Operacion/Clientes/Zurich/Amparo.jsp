...
<%  
    ...
    StrSql.append(" where V.clexpediente = ").append(StrclExpediente);
      ...
      StrSql.delete(0,StrSql.length());
       ...
       String StrclPaginaWeb = "586";
       session.setAttribute("clPaginaWebP",StrclPaginaWeb);
       %><SCRIPT>fnOpenLinks()</script><%


       MyUtil.InicializaParametrosC(586,Integer.parseInt(StrclUsrApp)); 
       %>
       <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="Amparo.jsp?'>"%><%   
       ...
       if (rs.next()) {
            ...
            <%=MyUtil.DoBlock("Amparo",70,0)%> 
            
            <%=MyUtil.ObjInput("*Fecha Amparo","FechaAmparo","",false,false,30,230,"",false,false,19)%>
            <%=MyUtil.ObjInput("*Fecha Resultado Amparo","FechaAmparoRes","",false,false,300,230,"",false,false,19)%>
            <%=MyUtil.ObjTextArea("*Resultado Amparo","AmparoRes","","90","4",true,true,30,270,"",true,true)%>
            ...
            <%=MyUtil.DoBlock("Datos del Amparo",70,-10)%> 
            
            <%=MyUtil.ObjTextArea("Informe de Terminación del Caso","Informe",rs.getString("Informe"),"90","4",true,true,30,430,"",true,true)%>                  
            <%=MyUtil.ObjInput("*Fecha Terminación","FechaConcluye","",false,false,30,510,"",false,false,19)%>
            <%=MyUtil.ObjChkBox("*Tercero Inicia Proceso Civil","ProcesoCivilCP","", true,true,300,520,"0","")%>
