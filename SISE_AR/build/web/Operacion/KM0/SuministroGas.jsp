...
<html>
    <head>
        <title>Suministro de Combustible</title>
    ...
    <body class="cssBody">
        ...
        <%MyUtil.InicializaParametrosC(200, Integer.parseInt(StrclUsrApp));%>
        <%=MyUtil.doMenuAct("../../servlet/Utilerias.EjecutaAccionAsist", "fnValLugar();fnLlenaComboAuto();", "fnValLugar();", "")%>

        <INPUT id='URLBACK' name='URLBACK' type='hidden' value='<%=request.getRequestURL().substring(0, request.getRequestURL().lastIndexOf("/") + 1)%><%="SuministroGas.jsp?"%>'>
        ...
        <%=MyUtil.ObjInput("Modelo", "Modelo", AV != null ? AV.getModelo() : "", true, true, 450, 330, StrModelo, true, true, 6, "if(this.readOnly==false){fnValidaModelo(this)}")%>
        <%=MyUtil.ObjInput("Color", "Color", AV != null ? AV.getColor() : "", true, true, 520, 330, StrColor, true, true, 10)%>
        <%=MyUtil.ObjInput("Patente", "Placas", AV != null ? AV.getPatente() : "", true, true, 610, 330, StrPlacas, true, true, 8)%>
        <%=MyUtil.ObjComboC("Tipo de Gasolina", "clTipoGasolina", AV != null ? AV.getDsTipoGas() : "", true, true, 30, 368, "", "Select clTipoGasolina, dsTipoGasolina from cTipoGasolina order by dsTipoGasolina ", "", "", 30, true, true)%>
        <%=MyUtil.ObjInput("Litros", "Litros", AV != null ? AV.getLitros() : "", true, true, 195, 368, "", false, false, 5, "if(this.readOnly==false){fnValMask(this,document.all.LitrosMsk.value,this.name)}")%>
        <%//=MyUtil.ObjComboC("Lugar", "clLugarEvento", AV != null ? AV.getDsLugar() : "", true, true, 260, 368, "", "select clLugarEvento, dsLugarEvento from cLugarEvento order by dsLugarEvento", "", "", 20, true, true)//%>
        <%=MyUtil.ObjComboC("Lugar", "clLugarEvento", AV != null ? AV.getDsLugar() : "", false, false, 260, 368, "", "st_LugarEvento", "", "", 20, false, false)%>
        <INPUT id='ClaveAMISVTR' name='ClaveAMISVTR' type='hidden' value=''>
        <%=MyUtil.DoBlock("Detalle de Suministro de Combustible", -115, -10)%>
        ...
        <script type="text/javascript">
            ...
            // AUTOMATIZA LUGAR A NO CARRETERO. JOMU
