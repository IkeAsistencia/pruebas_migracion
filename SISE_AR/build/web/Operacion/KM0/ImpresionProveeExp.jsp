...
<html>
<head><title>IMPRESION DETALLE SOLICITUD</title> 
...
//    StrSql = "sp_WebSolicitudServicioProv " + StrclUsrApp + "," + StrclExpediente + "," + StrclProveedor;
   ResultSet rs = UtileriasBDF.rsSQLNP( "sp_WebSolicitudServicioProv " + StrclUsrApp + "," + StrclExpediente + "," + StrclProveedor);
   %>
    <form id='Forma' name ='Forma'  action='ImpresionProveeExp.jsp?' method='get'>
    <%
    if (rs.next())
    {
        %>
        <table class='TTable'>
        <tr><th colspan=4>DETALLE DE LA SOLICITUD DEL SERVICIO</th></tr><tr></tr><tr></tr>
        <%
	iCont = 2;
	while(9 > iCont){
            %>
	   ...
	   iCont = iCont + 2;
           %>
	   <td width='120'><%=rs.getString(iCont)%>: </td><td class='R1Table' width='260'><%=rs.getString(iCont-1)%></td>
	   </tr>
	   <%
	   iCont = iCont + 2;
	}
	...
	// entidad
       ...
       // destino
           %>
	   <tr><td width='190'><%=rs.getString(iCont)%>: </td><td colspan=3 class='R1Table' ><%=rs.getString(iCont-1)%></td></tr>
	   <%
   	   iCont = iCont + 2;
           %>
           <tr></tr><tr></tr>
