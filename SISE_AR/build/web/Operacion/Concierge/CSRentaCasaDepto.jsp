...
<body class="cssBody" onload="fnLimpiaFechas();fnVerificaFecha();if (document.all.ExpiraVTR.value!=''){fnFechVen(document.all.ExpiraVTR.value)}">
    ...
    <%=MyUtil.ObjChkBox("Niños","Ninos",conciergerentacasadepto!=null ? conciergerentacasadepto.getNinos() : "",true,true,130,220,"","SI","NO","EdadesNinos();")%>
    ...
    <%=MyUtil.ObjInput("Costo por Día","CostoxDia",conciergerentacasadepto!=null ? conciergerentacasadepto.getCostoxDia() : "",true,true,30,380,"",false,false,20,"EsNumerico(this)")%>
    ...
    <%=MyUtil.ObjInput("Pol.Cancelación","Cancelacion",conciergerentacasadepto!=null ? conciergerentacasadepto.getCancelacion() : "",true,true,30,640,"",false,false,50)%>
...
function fnAntesGuardar(){
    if (document.all.clEstatus.value==10 ) {
        if (document.all.CostoxDia.value==0 ) { msgVal = msgVal + " Costo por Día";}
