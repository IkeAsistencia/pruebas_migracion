<%-- 
    Document   : CSGolfProgram
    Created on : 26/08/2011, 11:37:31 AM
    Author     : rfernandez
--%>

<%@ page contentType="text/html; charset=iso-8859-1" language="java" import="Seguridad.SeguridadC,Combos.cbEstatus,com.ike.concierge.DAOConciergeG,com.ike.concierge.to.ConciergeG,com.ike.concierge.DAOConciergeGolfProgram,com.ike.concierge.to.Conciergegolfprogram,com.ike.concierge.DAOReferenciasxAsist,com.ike.concierge.to.ReferenciasxAsist" errorPage="" %>
<%@ page import = "java.sql.ResultSet,Utilerias.UtileriasBDF"%>

<html>
    <head><title>Campos de Golf Program</title>
    ...
    <div class='VTable' style='position:absolute; z-index:25; left:400px; top:15px;'><p align="center"><font color="navy" face="Arial" size="3" ><b><i>Campos de Golf Program</i></b>  <br> </p></div>
            ...
            DAOConciergeGolfProgram daos = null;
            Conciergegolfprogram conciergegolf = null;
            ...
            if (request.getParameter("clGolfProgram") != null) {
                ...
                session.setAttribute("clGolfProgram", StrclGolfProgram);
            } else {
                if (session.getAttribute("clGolfProgram") != null) {
                    StrclGolfProgram = session.getAttribute("clGolfProgram").toString();
                }
            }
            ...
            String StrclPaginaWeb = "1405";
            session.setAttribute("clPaginaWebP", StrclPaginaWeb);

            if (StrclUsr != null) {
                daos = new DAOConciergeGolfProgram();
            ...
            //<<<<<<<<<<<< Servlet Generico >>>>>>>>>>>
            String Store = "";
            Store = "st_GuardaCSGolfProgram,st_ActualizaCSGolfProgram";
            ...
            ResultSet rsP = null;
            String StrCampoRequeridos = "0";

            rsP = UtileriasBDF.rsSQLNP("st_CSValidaGolfPlatinum '" + StrclConcierge +"','"+ StrclCuenta +"'");
            if (rsP.next()) {
                StrCampoRequeridos = rsP.getString("CamposRequeridos").toString();
            }
        ...
        %>
        <%MyUtil.InicializaParametrosC(1405, Integer.parseInt(StrclUsr));%>
        ...
        <input id="SecuenciaG" name="SecuenciaG" type="hidden" value="clConcierge,FechaApAsist,clUsrApp,Jugadores,Fecha,Green,Equipo,Diestros,Zurdos,OtrosC,CargoT,Hotel,FechaI,Reservacion,FechaO,clTipoPago,NomBanco,NombreTC,NumeroTC,ExpiraVTR,SecC,Confirmo,NConfirmo,PCancel,NuInf,Comentarios,Handicap,Aprobada,Aprovacion">
        <input id="SecuenciaA" name="SecuenciaA" type="hidden"                      VALUE="clAsistencia,Jugadores,Fecha,Green,Equipo,Diestros,Zurdos,OtrosC,CargoT,Hotel,FechaI,Reservacion,FechaO,clTipoPago,NomBanco,NombreTC,NumeroTC,ExpiraVTR,SecC,Confirmo,NConfirmo,PCancel,NuInf,Comentarios,clEstatus,Handicap,Aprobada,Aprovacion">
        ...
        <INPUT id='CampoRequerido' name='CampoRequerido' type='hidden' value='<%=StrCampoRequeridos%>'>
        ...
        <div class='VTable'  id="btnGolf" style="position:absolute; z-index:25; left:560px; top:135px;">
            <input class='cBtn' type='button' value='Campos de Golf' onClick="fnAbrirCampoGolf();"></input>
        </div>
        <%=MyUtil.DoBlock("Datos Generales del Usuario", 10, 15)%>
        <%=MyUtil.ObjInput("Jugadores", "Jugadores", conciergegolf != null ? conciergegolf.getJugadores() : "", true, true, 30, 220, "", true, true, 25)%>
        <%=MyUtil.ObjInputF("Fecha <Strong>(AAAA-MM-DD hh:mm)</Strong>", "Fecha", conciergegolf != null ? conciergegolf.getFecha() : "", true, true, 250, 220, "", true, true, 20, 2, "")%>
        ...
        <%=MyUtil.ObjInputF("Check-In <Strong>(AAAA-MM-DD hh:mm)</Strong>", "FechaI", conciergegolf != null ? conciergegolf.getFechaI() : "", true, true, 450, 340, "", false, false, 20, 2, "fnVerificaFecha();")%>
        <%=MyUtil.ObjInput("Rva. a nombre", "Reservacion", conciergegolf != null ? conciergegolf.getReservacion() : "", true, true, 30, 380, "", false, false, 75)%>
        <%=MyUtil.ObjInputF("Check-Out <Strong>(AAAA-MM-DD hh:mm)</Strong>", "FechaO", conciergegolf != null ? conciergegolf.getFechaO() : "", true, true, 450, 380, "", false, false, 20, 2, "fnVerificaFecha();")%>
        ...
        <%=MyUtil.ObjInput("Nombre del Banco:", "NomBanco", conciergegolf != null ? conciergegolf.getNomBanco() : "", true, true, 200, 420, "", false, false, 40)%>
        <%=MyUtil.ObjInput("Nombre en TC:", "NombreTC", conciergegolf != null ? conciergegolf.getNombreTC() : "", true, true, 450, 420, "", false, false, 30)%>
        <%=MyUtil.ObjInput("Numero de TC:", "NumeroTC", conciergegolf != null ? conciergegolf.getNumeroTC() : "", true, true, 30, 460, "", false, false, 50, "if(this.readOnly==false){fnValMask(this,document.all.NumeroTCMsk.value,this.name)};fnValidaPrefijoTC(this.value);")%>
        <%=MyUtil.ObjInput("Exp.D.:(MM/AA)", "ExpiraVTR", conciergegolf != null ? conciergegolf.getExpira() : "", true, true, 350, 460, "", false, false, 10, "if(this.readOnly==false){fnValMask(this,document.all.ExpiraMsk.value,this.name)};fnFechVen(this.value);")%>
        <%=MyUtil.ObjInput("Sec.C.:", "SecC", conciergegolf != null ? conciergegolf.getSecC() : "", true, true, 440, 460, "", false, false, 10, "if(this.readOnly==false){fnValMask(this,document.all.SecCMsk.value,this.name)}")%>
        ...
        <%=MyUtil.ObjInput("Handicap", "Handicap", conciergegolf != null ? conciergegolf.getHandicap() : "", true, true, 550, 260, "", true, true, 20)%>
        <%=MyUtil.ObjChkBox("Aprobada", "Aprobada", conciergegolf != null ? conciergegolf.getAprobada() : "", true, true, 470, 540, "", "SI", "NO", "")%>
        <%=MyUtil.ObjInput("Aprobacion", "Aprovacion", conciergegolf != null ? conciergegolf.getAprovacion() : "", true, true, 570, 540, "", false, false, 25)%>

        <%=MyUtil.DoBlock("Reservación de Campos de Golf", 10, 0)%>
...
<script>
    ...
    function fnValidaFecha()
    {
       if (document.all.Action.value==2){
        ...
        if (document.all.FechaI.value!=' ' && document.all.FechaO!=' ')
        {
...
document.all.FechaO.value='';
    ...
    function fnAbrirCampoGolf(){
        if (document.all.Action.value==2){

            var pstrCadena = "CSListaCamposGolf.jsp?clAsistencia=" + document.all.clAsistencia.value;
            window.open(pstrCadena,'newWinNA','scrollbars=yes,status=yes,width=640,height=770,top=280,left=770');
        }
        else{
            alert("Debe de Ingresar los datos de la Asistencia");
        }
    }


    function fnPlatinumObliga(){


        if(document.all.CampoRequerido.value==1  ) {
            if(document.all.AprobadaC.checked==0){
               msgVal=msgVal + " Aprobada. "
               }

            if(document.all.Aprovacion.value==''){
               msgVal=msgVal + " Aprobacion. "
               }
            document.all.btnGuarda.disabled=false;
            document.all.btnCancela.disabled=false;
