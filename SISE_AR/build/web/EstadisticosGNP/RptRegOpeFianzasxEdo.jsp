...
<%@ page pageEncoding="iso-8859-1" errorPage=""%>
<%@ page import="java.sql.Timestamp,java.util.Calendar,java.util.Iterator,java.util.Map,java.util.LinkedHashMap,Utilerias.Grafica,java.sql.ResultSet,Utilerias.UtileriasBDF,Seguridad.SeguridadC" %>

<html>
    <head><title>Registro de la Operacion Fianzas x Estado</title>
    ...
    <body class="cssBody">
        ...
        %>Fuera de Horario<%
            ...
            return;
                ...
                MyUtil.InicializaParametrosC(670,Integer.parseInt(StrclUsrApp));
        %>
        <form name='frmBusq' id='frmBusq' method='post' action='../EstadisticosGNP/RptRegOpeFianzasxEdo.jsp'>
            ...
            <%=MyUtil.ObjComboC("Fianzas","clTipo","",true,true,340,30,"","Select '1' 'clTipo','Exhibidas' 'dsTipo' Union Select '2' 'clTipo','Recuperadas' 'dsTipo'","","",25,true,true)%>
        ...
        <%
            ...
            StrSql.append("st_GNPRptRegOpeXEdoFianzas '").append(StrclAnio).append("','").append(clMes).append("','").append(clTipo).append("'");
            StrQry="st_GNPRptRegOpeXEdoFianzas "+StrclAnio+","+clMes+","+clTipo;
        ...
        <div style='position:absolute; z-index:20; left:10px; top:120px;'> 
            ...
            <table width="100%" border="0" cellspacing="1" cellpadding="1">
                <tr>
                <td colspan='6' class="STableTit">FIANZAS <%=dsTipoExped%></td>
                </tr> 
                <td colspan='6' class='STableTexto'>&nbsp;</td>
                </tr>
                <tr> 
                    ...
                    <td colspan="2" class="STableTit"><%=Mes + " " + clAnio%></td>
                    <td  width="15%" class="STableTit"><%="ACUMULADO " +clAnio%></td>
                </tr>
                <tr> 
                    <td class='STableTit'>N&deg;</td>
                    <td width="15%" class='STableTit'>N&deg;</td>
                    <% if(clTipo.equalsIgnoreCase("1")){%> <td width="15%" class='STableTit'>MONTO</td>   <% } else {%> <td width="15%" class='STableTit'></td>   <%} %>
                ...
                <%                   
                    while(it.hasNext()){
                            ...
                            for(int rows = 1; rows <= Integer.parseInt(pvalue);rows++){
                                ...
                                <% if(clTipo.equalsIgnoreCase("1")){%> <td>$<%=rs.getString(6)%></td>   <% } else {%> <td></td>  <%} %>
                            ...
                            <% if(clTipo.equalsIgnoreCase("1")){%> <td class='STableTit'>$<%=rs.getString(6)%></td></td>   <% } else {%> <td class='STableTit'></td>  <%} %>
                ...
                %>
                            ...
                            <% if(clTipo.equalsIgnoreCase("1")){%><td class='STableTit'>$<%=rs.getString(6)%></td>   <% } else {%> <td class='STableTit'></td>  <%} %>
                            <td class='STableTit'><%=rs.getString(7)%></td>
                            </tr>                                
            </table>
        </div>
