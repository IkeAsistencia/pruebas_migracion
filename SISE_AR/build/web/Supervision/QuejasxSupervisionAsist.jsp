...
<html>
    ...
    <body class="cssBody" onload="fn_ChecaDano();">
        ...
        <%=MyUtil.ObjTextArea("Observaciones de Área","ObservacionesArea",rs.getString("ObservacionesArea"),"80","3",false,true,30,250,"",false,false)%>
        <%=MyUtil.ObjTextArea("SOLUCIÓN","Solucion",rs.getString("Solucion"),"80","3",false,true,30,320,"",false,false)%>
        <%=MyUtil.ObjComboC("Tipo de SOLUCIÓN","clTipoSolQueja",rs.getString("dsTipoSolQueja"),true,true,500,320,"","Select clTipoSolQueja,dsTipoSolQueja from cTipoSolQueja","","",100,false,false)%>
        ...
        <%=MyUtil.ObjInput("Fecha de SOLUCIÓN<BR>AAAA/MM/DD HH:MM","FechaSolucion",rs.getString("FechaSolucion"),false,false,300,390,"",false,false,22,"")%>
        <%=MyUtil.DoBlock("Módulo de Quejas",50,25)%>
        <% if (rs.getString("clQueja").equalsIgnoreCase("1")){ %>
        <div id="DetalleDanos">
            <%=MyUtil.ObjComboC("Tipo de Daño","clDano",rs.getString("dsDano"),false,true,30,620,"0","Select clDano , dsDano  from cDanos","fn_LLenaSubDano();","",50,true,true)%>
            <%=MyUtil.ObjComboC("SubTipo de Daño","clSubDano",rs.getString("dsSubDano"),false,true,200,620,"0","sp_TraeSubDano '"+rs.getString("clDano")+"'","","",50,true,true)%>
            <%=MyUtil.ObjChkBox("ACEPTACIÓN","clAceptacionDano",rs.getString("clAceptacionDano"), true,true,370,620,"0","SI","NO","fn_AceptaDano(clAceptacionDano.value);")%>                
            ...
            <%=MyUtil.ObjComboC("Modo Reparación","clModoReparacionDano",rs.getString("dsModoReparacionDano"),false,true,250,670,"0","Select clModoReparacionDano , dsModoReparacionDano  from cModoReparacionDano ","","",50,false,false)%>                               
            ...
            <%=MyUtil.ObjComboC("Comprobante de REPARACIÓN","clComprobanteRepDano",rs.getString("dsComprobanteRepDano"),false,true,250,783,"0","select clComprobanteRepDano,dsComprobanteRepDano from cComprobanteRepDano","","",50,false,false)%>
        ...
        <div class='VTable' id ="AceptaDano" style='position:relative; z-index:200; left:0px; top:0px; visibility: hidden '>
            <%=MyUtil.ObjInputF("Fecha Aceptación","FechaAceptacionDano",rs.getString("FechaAceptacionDano"),false,false,470,580,"",false,false,20,1,"")%>
        </div>
        ...
        <%=MyUtil.ObjTextArea("Observaciones de Área","ObservacionesArea","","80","3",false,true,20,200,"",false,true)%>
        <%=MyUtil.ObjTextArea("SOLUCIÓN","Solucion","","80","3",false,true,20,270,"",false,true)%>
        <%=MyUtil.ObjComboC("Tipo de SOLUCIÓN","clTipoSolQueja","",true,true,490,280,"","Select clTipoSolQueja,dsTipoSolQueja from cTipoSolQueja","","",100,false,false)%>
        <%=MyUtil.ObjInput("Fecha de Ingreso","FechaIngresoVTR","",false,false,20,350,StrFecha,true,true,22)%>
        <%=MyUtil.ObjInput("Fecha de SOLUCIÓN<BR>AAAA/MM/DD HH:MM","FechaSolucion","",false,false,290,340,"",false,false,22,"")%>
        <%=MyUtil.DoBlock("Módulo de Quejas",0,0)%>
        ...
        <div style="visibility:hidden">
            ...
            <%=MyUtil.ObjComboC("Tipo de Daño","clDano","",false,true,30,620,"0","Select clDano , dsDano  from cDanos","fn_LLenaSubDano();","",50,false,false)%>
            <%=MyUtil.ObjComboC("SubTipo de Daño","clSubDano","",false,true,200,620,"0","sp_TraeSubDano","","",50,false,false)%>
            <%=MyUtil.ObjChkBox("Aceptación","clAceptacionDano","", true,true,370,620,"0","SI","NO","fn_AceptaDano(clAceptacionDano.value);")%>                
            ...
            <%=MyUtil.ObjComboC("Modo Reparación","clModoReparacionDano","",false,true,250,670,"0","Select clModoReparacionDano , dsModoReparacionDano  from cModoReparacionDano ","","",50,false,false)%>                               
            ...
            <%=MyUtil.ObjComboC("Comprobante de Reparación","clComprobanteRepDano","",false,true,250,783,"0","select clComprobanteRepDano,dsComprobanteRepDano from cComprobanteRepDano","","",50,false,false)%>
            <%=MyUtil.ObjInputF("Fecha Comprobante REPARACIÓN","FechaRegistroComprob","",false,false,470,783,"",false,false,20,1,"")%>
            <%=MyUtil.ObjInputF("Fecha Aceptación","FechaAceptacionDano","",false,false,470,580,"",false,false,20,1,"")%>
        ...
        <script>
            ...
            function fnObligatorio(){
                if(document.all.clEstatusQueja.value == 4){
                    alert('Seleccione una notificación de solución');
            ...
            function fnRequiere(){
                ...
                if(document.all.clTipoSolQueja.value=="" && (document.all.clEstatusQueja.value==4 || document.all.clEstatusQueja.value==6)){
                 ...
                 if(document.all.clAceptacionDanoC.checked==1){
                              msgVal= msgVal + ' Fecha Aceptación.';
                              document.all.btnGuarda.disabled=false;
                              document.all.btnCancela.disabled=false;
                 }*/
            }
