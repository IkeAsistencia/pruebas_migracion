...
<html>
    <head>
        <title>Detalle Proveedor Funerario</title>
    </head>
    <body class="cssBody">
        ...
        <%
            MyUtil.InicializaParametrosC(478, Integer.parseInt(StrclUsr));
        %>
        <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion", "fnLimpiaExtra()")%>
        <INPUT id='URLBACK' name='URLBACK' type='hidden' value='<%=request.getRequestURL().substring(0, request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleProveedorF.jsp?"%>'>
        <%
            if (rs.next()) {
        ...
        <INPUT id='clAreaOperativa' name='clAreaOperativa' type='hidden' value='10'>
        ...
        <%=MyUtil.ObjComboC("Rubro", "clGiro", rs.getString("dsGiro"), true, true, 310, 120, "", "Select clGiro, dsGiro from cGiro where clGiro = 15", "", "", 10, true, true)%>
        <%=MyUtil.ObjChkBox("", "Activo", rs.getString("Activo"), true, true, 480, 160, "", "ACTIVO", "INACTIVO", "")%>
        ...
        <%=MyUtil.ObjInput("Horario", "Horario", rs.getString("Horario"), true, true, 310, 160, "", false, false, 30)%>
        <%=MyUtil.ObjComboC("<br>Tipo de Proveedor", "clTipoProveedor", rs.getString("dsTipoProveedor"), true, true, 25, 200, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 10 order by dsTipoProveedor", "", "", 20, true, true)%>
        <%=MyUtil.ObjInput("<br>Prioridad", "Prioridad", rs.getString("Prioridad"), true, true, 200, 200, "", true, true, 5)%>
        <%=MyUtil.ObjInput("Fecha de contrtación<br>(aaaa/mm/dd)", "FechaContratacion", rs.getString("FechaContratacion"), true, true, 310, 200, "", false, false, 20, "if(this.readOnly==false){fnValMask(this,document.all.FechaContrataMsk.value,this.name)};")%>
        <%=MyUtil.ObjInput("Fecha de Alta<br>(aaaa/mm/dd)", "FechaAltaVTR", rs.getString("FechaAlta"), false, false, 465, 200, "", false, false, 20, "if(this.readOnly==false){fnValMask(this,document.all.FechaContrataMsk.value,this.name)};")%>
        <%=MyUtil.DoBlock("Detalle de Proveedor (FUNERARIO)")%>
        <%yPos = 320;%>
        ...
        <%=MyUtil.ObjInput("Cuit", "CP", rs.getString("CP"), true, true, 315, yPos, "", false, false, 10)%>
        ...
        <%=MyUtil.ObjInput("Num. de Sucursal", "SucursalBan", rs.getString("SucursalBan"), true, true, 350, yPos, "", false, false, 15)%>
        ...
        <%yPos += 40;%>
        <%=MyUtil.ObjChkBox("Copia de Identificación del responsable y técnicos(Opcional)", "CopiaIdent", rs.getString("CopiaIdent"), true, true, 30, yPos, "0", "")%>
        <%yPos += 40;%>
        ...
        <%yPos += 40;%>
        <%=MyUtil.ObjChkBox("Fotografías del negocio(Opcional)", "Fotografias", rs.getString("Fotografias"), true, true, 30, yPos, "0", "")%>
        <%yPos += 40;%>
        <%=MyUtil.ObjChkBox("Aprobado", "Aprobado", rs.getString("Aprobado"), true, true, 30, yPos, "0", "")%>
        ...
        <%=MyUtil.ObjComboC("Rubro", "clGiro", "", true, true, 310, 120, "", "Select clGiro, dsGiro from cGiro where clGiro = 15", "", "", 10, true, true)%>
        <%=MyUtil.ObjChkBox("", "Activo", "", true, true, 480, 160, "", "ACTIVO", "INACTIVO", "")%>
        ...
        <%=MyUtil.ObjInput("Horario", "Horario", "", true, true, 310, 160, "", false, false, 30)%>
        <%=MyUtil.ObjComboC("<br>Tipo de Proveedor", "clTipoProveedor", "", true, true, 25, 200, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 10 order by dsTipoProveedor", "", "", 20, true, true)%>
        <%=MyUtil.ObjInput("<br>Prioridad", "Prioridad", "", true, true, 200, 200, "", true, true, 5)%>
        <%=MyUtil.ObjInput("Fecha de contrtación<br>(aaaa/mm/dd)", "FechaContratacion", "", true, true, 310, 200, "", true, true, 30, "if(this.readOnly==false){fnValMask(this,document.all.FechaContrataMsk.value,this.name)};")%>
        <%=MyUtil.ObjInput("Fecha de Alta<br>(aaaa/mm/dd)", "FechaAltaVTR", "", false, false, 465, 200, "", false, false, 20, "if(this.readOnly==false){fnValMask(this,document.all.FechaContrataMsk.value,this.name)};")%>
        <%=MyUtil.DoBlock("Detalle de Proveedor (FUNERARIO)")%>
        <%yPos = 320;%>
        ...
        <%=MyUtil.ObjComboC("Banco", "clBanco", "", true, true, 25, yPos, "", "Select * from cBanco where Activo=1 order by Nombre", "", "", 50, false, false)%>
        ...
        <%=MyUtil.ObjInput("Núm. de Cuenta", "CuentaBan", "", true, true, 325, yPos, "", false, false, 20)%>
        <%=MyUtil.ObjInput("C.U.B.", "Clabe", "", true, true, 460, yPos, "", false, false, 20, "if(this.readOnly==false){fnValMask(this,document.all.CubMsk.value,this.name)}")%>
        ...
        <%=MyUtil.DoBlock("Banco del Proveedor", 0, 0)%>
        <%yPos += 115;%>
        <%=MyUtil.ObjTextArea("Observaciones", "Observaciones", "", "100", "7", true, true, 25, yPos, "", false, false)%>
        <%=MyUtil.DoBlock("Observaciones Extras", 400, 70)%>
        ...
        <%yPos += 40;%>
        <%=MyUtil.ObjChkBox("Copia de Identificación del responsable y técnicos(Opcional)", "CopiaIdent", "", true, true, 30, yPos, "0", "")%>
        <%yPos += 40;%>
        ...
        <%yPos += 40;%>
        <%=MyUtil.ObjChkBox("Fotografías del negocio(Opcional)", "Fotografias", "", true, true, 30, yPos, "0", "")%>
        <%yPos += 40;%>
        <%=MyUtil.ObjChkBox("Aprobado", "Aprobado", "", true, true, 30, yPos, "0", "")%>
        <%=MyUtil.DoBlock("Requisitos de Aprobación", 300, 0)%>
        <%
            }
        %>
        ...
        <input name='FechaContrataMsk' id='FechaContrataMsk' type='hidden' value='VN09VN09VN09VN09F-/-VN09VN09F-/-VN09VN09'>
    </body>
</html>