...
<%  
    ...
    StrSql1.append("select TC.dsTipoContacto, CxP.Contacto, TC.clTipoContacto, ");
StrSql1.append(" CxP.clContactoxPersonal ");
...
ResultSet rs = UtileriasBDF.rsSQLNP( StrSql1.toString());
    StrSql1.delete(0,StrSql1.length());
       ...
       String StrclPaginaWeb = "497";
       session.setAttribute("clPaginaWebP",StrclPaginaWeb);
       %>
       <SCRIPT>fnOpenLinks()</script>
       <%
        MyUtil.InicializaParametrosC(497,Integer.parseInt(StrclUsrApp));    // se checan permisos de alta,baja,cambio,consulta de esta pagina 
        %>
       <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion","")%>
       <%="<INPUT id='URLBACK' name='URLBACK' type='hidden' value='"%><%=request.getRequestURL().substring(0,request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleContxPersF.jsp?'>"%>
       ...
       if (rs.next()) {
           %>
            ...
            %>
          <%=MyUtil.DoBlock("Detalle de Medio de Contacto (FUNERARIO)",190,0)%>
