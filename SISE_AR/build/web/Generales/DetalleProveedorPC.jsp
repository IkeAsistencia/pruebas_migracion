...
<html>
    <head>
        <title>Detalle Proveedor PC</title>
    ...
    <body class="cssBody">
        ...
        <%
            MyUtil.InicializaParametrosC(768, Integer.parseInt(StrclUsr));
        %>
        <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion", "fnLimpiaExtra()")%>
        <INPUT id='URLBACK' name='URLBACK' type='hidden' value='<%=request.getRequestURL().substring(0, request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleProveedorPC.jsp?"%>'>
        <%
            if (rs.next()) {
        ...
        %>
        <INPUT id='clAreaOperativa' name='clAreaOperativa' type='hidden' value='6'>
        ...
        <%=MyUtil.ObjComboC("Rubro", "clGiro", rs.getString("dsGiro"), true, true, 310, 120, "", "Select clGiro,dsGiro from cGiro where clAreaOperativa=6 order by dsGiro", "", "", 10, true, true)%>
        <%=MyUtil.ObjChkBox("", "Activo", rs.getString("Activo"), true, true, 480, 200, "", "ACTIVO", "INACTIVO", "")%>
        <%=MyUtil.ObjInput("Cuit", "RFC", rs.getString("RFC"), true, true, 25, 160, "", false, false, 20)%>
        ...
        <%=MyUtil.ObjComboC("Tipo de Proveedor", "clTipoProveedor", rs.getString("dsTipoProveedor"), true, true, 25, 200, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 6 order by dsTipoProveedor", "", "", 20, true, true)%>
        <%=MyUtil.ObjInput("Prioridad", "Prioridad", rs.getString("Prioridad"), true, true, 200, 200, "", true, true, 5)%>
        <%=MyUtil.ObjInput("Fecha&nbsp;de&nbsp;Alta&nbsp;(aaaa/mm/dd)", "FechaAltaVTR", rs.getString("FechaAlta"), false, false, 310, 200, "", false, false, 20, "if(this.readOnly==false){fnValMask(this,document.all.FechaContrataMsk.value,this.name)};")%>
        <%=MyUtil.DoBlock("Detalle de Proveedor de Asistencia PC")%>
        <%yPos = 330;%>
        <%
        ...
        %>
        <%=MyUtil.ObjComboMem("Provincia", "CodEnt", dsEntFed, CodEnt, cbEntidad.GeneraHTML(20, dsEntFed), true, true, 25, 290, "", "fnLlenaMunicipios()", "", 20, true, true)%>
        <%=MyUtil.ObjComboMem("Localidad", "CodMD", dsMunDel, StrCodMD, cbEntidad.GeneraHTMLMD(30, CodEnt, dsMunDel), true, true, 410, 290, "", "", "", 20, true, true)%>
        ...
        <%=MyUtil.ObjInput("Email 1", "Email1", rs.getString("Email1"), true, true, 25, 515, "", false, false, 40)%>
        <%=MyUtil.ObjInput("Email 2", "Email2", rs.getString("Email2"), true, true, 250, 515, "", false, false, 40)%>
        <%=MyUtil.ObjInput("Email 3", "Email3", rs.getString("Email3"), true, true, 475, 515, "", false, false, 40)%>
        ...
        <%=MyUtil.DoBlock("Requisitos de Aprobación (IF)", 300, 0)%>
        <%
        ...
        <%=MyUtil.ObjComboC("Rubro", "clGiro", "", true, true, 310, 120, "", "Select clGiro, dsGiro from cGiro where clAreaOperativa = 6 order by dsGiro", "", "", 10, true, true)%>
        <%=MyUtil.ObjChkBox("", "Activo", "", true, true, 480, 200, "", "ACTIVO", "INACTIVO", "")%>
        <%=MyUtil.ObjInput("CUIT", "RFC", "", true, true, 25, 160, "", false, false, 20)%>
        ...
        <%=MyUtil.ObjComboC("Tipo de Proveedor", "clTipoProveedor", "", true, true, 25, 200, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 6 order by dsTipoProveedor", "", "", 20, true, true)%>
        <%=MyUtil.ObjInput("Prioridad", "Prioridad", "", true, true, 200, 200, "", true, true, 5)%>
        <%=MyUtil.ObjInput("Fecha&nbsp;de&nbsp;Alta&nbsp;(aaaa/mm/dd)", "FechaAltaVTR", "", false, false, 310, 200, "", false, false, 20, "if(this.readOnly==false){fnValMask(this,document.all.FechaContrataMsk.value,this.name)};")%>
        <%=MyUtil.DoBlock("Detalle de Proveedor de Asistencia PC")%>
        <%yPos = 320;%>
        ...
        <%yPos += 40;%>
        <%=MyUtil.DoBlock("Requisitos de Aprobación (ELSE)", 300, 0)%>
