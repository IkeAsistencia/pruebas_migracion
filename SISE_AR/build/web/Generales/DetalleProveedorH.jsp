...
<html>
    <head>
        <title>Detalle Proveedor Hogar</title>
    ...
    <body class="cssBody">
        ...
        <%
            MyUtil.InicializaParametrosC(242, Integer.parseInt(StrclUsr));
        %>
        <%=MyUtil.doMenuAct("../servlet/Utilerias.EjecutaAccion", "fnLimpiaExtra()")%>
        <INPUT id='URLBACK' name='URLBACK' type='hidden' value='<%=request.getRequestURL().substring(0, request.getRequestURL().lastIndexOf("/") + 1)%><%="DetalleProveedorH.jsp?"%>'>
        <%
            if (rs.next()) {
                StrNomOpe = rs.getString("NombreOpe");
                session.setAttribute("NombreOpe", StrNomOpe);
        %>
        <INPUT id='clAreaOperativa' name='clAreaOperativa' type='hidden' value='4'>
        ...
        <%=MyUtil.ObjComboC("Rubro", "clGiro", rs.getString("dsGiro"), true, true, 310, 120, "", "Select clGiro, dsGiro from cGiro where clAreaOperativa = 4 order by dsGiro", "", "", 45, true, true)%>
        ...
        <%=MyUtil.ObjComboC("Tipo de Proveedor", "clTipoProveedor", rs.getString("dsTipoProveedor"), true, true, 25, 200, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 4 order by dsTipoProveedor", "", "", 20, true, true)%>
        ...
        <%=MyUtil.DoBlock("Detalle de Proveedor de Hogar")%>
        <%yPos = 320;%>
        ...
        <%=MyUtil.ObjInput("C.B.U.", "Clabe", rs.getString("Clabe"), true, true, 460, yPos, "", false, false, 32, "if(this.readOnly==false){fnValMask(this,document.all.CubMsk.value,this.name)}")%>
        ...
        <%=MyUtil.ObjComboC("Rubro", "clGiro", "", true, true, 310, 120, "", "Select clGiro, dsGiro from cGiro where clAreaOperativa = 4 order by dsGiro", "", "", 45, true, true)%>
        ...
        <%=MyUtil.ObjComboC("Tipo de Proveedor", "clTipoProveedor", "", true, true, 25, 200, "", "Select clTipoProveedor, dsTipoProveedor from cTipoProveedor where clAreaOperativa = 4 order by dsTipoProveedor", "", "", 20, true, true)%>
        ...
        <%=MyUtil.ObjComboMem("Prioridad", "CodEnt", "", "", cbEntidad.GeneraHTML(20, ""), true, true, 25, 280, "", "fnLlenaMunicipios()", "", 20, true, true)%>
        ...
        <%=MyUtil.ObjInput("C.U.B", "Clabe", "", true, true, 460, yPos, "", false, false, 32, "if(this.readOnly==false){fnValMask(this,document.all.CubMsk.value,this.name)}")%>
