...
public class cbGrupoCuenta {
    ...
    private synchronized static boolean loadComboList()
    {
     if(isConfigured == false)
     {
         ...
         try{
             ...
             ResultSet rs = UtileriasBDF.rsSQLNP("SELECT clGrupoCuenta clLlave ,dsGrupoCuenta dsDesc FROM cGrupoCuenta order by dsGrupoCuenta");
             while(rs.next())
             {
         ...
         {
             System.out.println("Entre a Load de Grupo Cuenta");
         }
     }
         return true;
    }
    ...
    public static String GeneraHTML(int pSize, String pValue){
        ...
        HashMap hshMap=cbGrupoCuenta.getComboHash();
