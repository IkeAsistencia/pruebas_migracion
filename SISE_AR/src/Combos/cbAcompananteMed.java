...
public class cbAcompananteMed {
    ...
    private synchronized static boolean loadComboList()
    {
     if(isConfigured == false)
     {
        comboHash = new HashMap();
         ...
         try{
             ResultSet rs = UtileriasBDF.rsSQLNP("SELECT clAcompananteMed clLlave ,dsAcompananteMed dsDesc FROM cAcompananteMed order by dsAcompananteMed");
             while(rs.next())
             {
         ...
         {
             System.out.println("Entre a Load de Acompanante Medico");
         }
     }
         return true;
    }
    ...
    public static String GeneraHTML(int pSize, String pValue){
        ...
        HashMap hshMap=cbAcompananteMed.getComboHash();
