...
public class cbParentesco {
    ...
    private synchronized static boolean loadComboList()
    {
     if(isConfigured == false)
     {
         ...
         try{
             ...
             ResultSet rs = UtileriasBDF.rsSQLNP("SELECT clParentesco clLlave ,dsParentesco dsDesc FROM cParentesco order by dsParentesco");
             while(rs.next())
             {
         ...
         {
             System.out.println("Entre a Load de Parentesco");
         }
      }
         return true;
    }
    ...
    public static String GeneraHTML(int pSize, String pValue){
        ...
        HashMap hshMap=cbParentesco.getComboHash();
