...
@Path("/citas")
public class AsigDirectaNoResponde {

    @PUT 
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @Path("/{clcita}/ad_no_responde/")
    public Response rechazar(@PathParam("clcita") int clCita) {
        System.out.println("EndPoint:/{clCita}/ad_no_responde/ :" + String.valueOf(clCita));
        ...
        try {
            ...
            if ( clExpediente.intValue() != 0 ) {
                ...
                String parms = clExpediente.toString() + ", " 
                                + "7, "      // @pclEstatus as int 5 -> Rechazo.
                                + "0, "      // @pclMotivo as int = 0,            
                                + "'PVD No Responde',"      // @pObservaciones as varchar(max),            
