...
//                    StrSQLBitacora.append("Insert into ").append(rs.getString("TablaBitacora")).append(" (clExpediente, FechaBitacora, clUsrApp, Bitacora) values (").append(sessionH);
                    ...
                    //System.out.println("impresion"+StrSentence);
                    while (rsInfo.next()) {
                ...
                if ((Integer.parseInt(request.getParameter("Action")) == 1) && (blnBackId == true)) {
                    System.out.println("Ejecuta Accion: " + StrSentence); // AGREGO MARIO PARA VER CLIENTESVIP
                    ...
                    System.out.println("Ejecuta Accion: " + StrSentence.toString()); // AGREGO MARIO PARA VER CLIENTESVIP
                    UtileriasBDF.ejecutaSQLNP(StrSentence.toString());
                }

                if (strTablaBitacora.compareToIgnoreCase("") != 0) {
                    InsertaBitacora(request, strTablaBitacora, strLlave, strNamIdentity);
                }
                String strUrlBack = "";
                ...
                System.out.println("Problemas al obtener informaci?n de la p?gina solicitada:   " + "select Tabla, coalesce(TablaBitacora,'') TablaBitacora from cPaginaWeb where clPaginaWeb = " + sessionH.getAttribute("clPaginaWebP"));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void InsertaBitacora(HttpServletRequest request, String pstrTabla, String pstrValIdentity, String pStrNamIdentity)
        ...
        try {
            ...
            //System.out.println("impresion2"+StrSentence);
            while (rsInfo.next()) {
                strNameF = rsInfo.getString("NameF");
