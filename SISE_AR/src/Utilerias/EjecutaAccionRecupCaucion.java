 ...
 * @version
 */
public class EjecutaAccionRecupCaucion extends HttpServlet {
        ...
        try{
            ...
            if(rs.next()){
                ...
                if ((Integer.parseInt(request.getParameter("Action"))==1)&&(blnBackId==true)){                
                    try
                    {
                           ResultSet rsValida = UtileriasBDF.rsSQLNP("sp_ValidaMontoRecup " + request.getParameter("clCaucion") + ","  + request.getParameter("MontoRecuperado") + ",0");                      
                           if (rsValida.next())
                           { 
                             ...
                             {
                                out.println("El Monto Recuperado excede el saldo por recuperar de la caución");
                                out.close();
                                return;
                             }
                           }    
                    ...
                    if ((Integer.parseInt(request.getParameter("Action"))==2)){  //en update validamos montos recuperados tambien
                           strclCaucion=request.getParameter("clCaucion");
                           ResultSet rsValida = UtileriasBDF.rsSQLNP("sp_ValidaMontoRecup " + strclCaucion + ","  + request.getParameter("MontoRecuperado") + "," + request.getParameter("clRecuperacion"));                      
                           if (rsValida.next())
                           { 
                             if (rsValida.getString("Resultado").equalsIgnoreCase("SI"))   // Si el Monto no excede de la Caucion, insertamos registro
                             {    
